...
final class DefaultModbusRequestResponseService[Req <: ModbusRequest[Resp], Resp <: ModbusResponse](
  ...
  def request(slaveId: SlaveId, request: Req): ZIO[Any, ModbusRequestResponseService.Error, request.Response] = {
    ...
    for {
      // send to serial line:
      _ <-
        serialPortService.writeBytes(allBytes).orElse(ZIO.dieMessage("Could not write bytes")) // TODO: error handling
      _ <- Clock.sleep(200.millis)
      // read response
      responseBytesWithCRC <- serialPortService.readAllAvailableBytesButAtLeastOneByte
                                .debugBy(_.length.toString)
                                .orElse(ZIO.dieMessage("Could not read response bytes")) // TODO: error handling
      // TODO: fail if no bytes read or better less than for at least an error response
      responseBytesWithoutCRC <-
        ZIO.fromOption(CRC16.check(responseBytesWithCRC)).orElseFail(ModbusRequestResponseService.Error.CRC)
      response <- ZIO
                    .fromEither(request.createResponseFromBytes(responseBytesWithoutCRC))
                    .mapError(ModbusRequestResponseService.Error.Modbus)
    } yield response
  }
}
