...
object CRC16 {
  def apply(bytes: Chunk[Byte]): CRC16 = {
    ...
    var low: Byte  = 0xff.toByte
    var high: Byte = 0xff.toByte
    bytes.foreach { b =>
  ...
  // None if check failed, Some input bytes without last two CRC bytes:
  def check(bytesWithCRC: Chunk[Byte]): Option[Chunk[Byte]] = {
    // TODO: return None or third channel (option has 2 channels) otherwise better message
    require(bytesWithCRC.length >= 2)

    val (payloadBytes, crcBytes) = bytesWithCRC.splitAt(bytesWithCRC.length - 2)
